...
class solucionTicket{
	...
	//Funcion para mostrar la foto según el usuario logeado
	public function MostrarFoto($idUsuario){
		$sqlFoto = "select foto from tecnico where idUsuario = '$idUsuario'";
		$imagen = $this->s->consultar($sqlFoto);

		while($row = mysqli_fetch_assoc($imagen)){
				echo '<img class="img-thumbnail" src="'.$row["foto"].'" alt="" id="logoEmpresa">';
			}
		}
